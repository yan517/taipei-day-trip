function getUserStatus(cb,value){
	const status = async () => await fetch(`/api/user/auth`)
...
loginBtn.addEventListener('click',(event) => {
    ...
    const login = async () => await fetch(`/api/user/auth`,{
        method:'PUT',
        headers:{
    ...
    const createAc = async () => await fetch(`/api/user`,{
        method:'POST',
        headers:{
            'Content-Type': 'application/json'
        },
...
appointment.addEventListener('click', (event) =>{
    const status = () => fetch(`/api/user/auth`)
	.then((response)=>response.json())
	.then((data =>{
		if(data["error"] || data["data"] == null){
            const loginSignUp = document.getElementById('loginSignup');
